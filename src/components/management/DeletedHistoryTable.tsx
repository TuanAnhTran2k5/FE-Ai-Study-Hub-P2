  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
        ...
        <p className="mt-2 text-sm">{t("curriculum.loadingDeleted", "Loading deleted history...")}</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div className="flex max-w-md items-center gap-2">
        <div className="relative flex-1">
          ...
          <Input
            placeholder={t("curriculum.searchDeleted", "Search deleted combos or subjects...")}
            value={searchKeyword}
        ...
        <Badge variant="outline" className="h-11 rounded-xl px-3 py-3">
          {t("curriculum.total", "Total")}: {combos.length + subjects.length}
        </Badge>
      </div>
      ...
      {combos.length === 0 && subjects.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border py-16 text-center">
          ...
          <h4 className="mt-4 font-bold text-card-foreground">
            {t("curriculum.emptyDeletedTitle", "No Deleted History")}
          </h4>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("curriculum.emptyDeletedDesc", "Deleted combos and subjects will appear here.")}
          </p>
        </div>
      ) : (
        <>
          <section className="space-y-3">
            <h3 className="font-black text-card-foreground">
              {t("curriculum.deletedCombos", "Deleted Combos")} ({combos.length})
            </h3>
            <div className="overflow-hidden rounded-2xl border border-border bg-card">
              <Table>
                <TableHeader className="bg-secondary/40">
                  <TableRow>
                    <TableHead className="w-[160px] font-bold">{t("curriculum.colSemesterCode", "Combo Code")}</TableHead>
                    <TableHead className="font-bold">{t("curriculum.colSubjectName", "Combo Name")}</TableHead>
                    <TableHead className="w-[160px] font-bold">{t("curriculum.subjectsCount", "Subjects")}</TableHead>
                    <TableHead className="w-[130px] text-right font-bold">{t("curriculum.colActions", "Actions")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {combos.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="py-8 text-center text-muted-foreground">
                        {t("curriculum.emptyDeletedCombos", "No deleted combos.")}
                      </TableCell>
                    </TableRow>
                  ) : (
                    combos.map((combo) => (
                      <TableRow key={combo.comboId} className="bg-muted/20 opacity-60">
                        ...
                        <TableCell className="text-right">
                          ...
                          >
                            ...
                            {t("curriculum.btnRestore", "Restore")}
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))
          ...
          <section className="space-y-3">
            <h3 className="font-black text-card-foreground">
              {t("curriculum.deletedSubjects", "Deleted Subjects")} ({subjects.length})
            </h3>
            <div className="overflow-hidden rounded-2xl border border-border bg-card">
              <Table>
                <TableHeader className="bg-secondary/40">
                  <TableRow>
                    <TableHead className="w-[130px] font-bold">{t("curriculum.colSubjCode", "Subj Code")}</TableHead>
                  ...
                  {sortedSubjects.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="py-8 text-center text-muted-foreground">
                        {t("curriculum.emptyDeletedSubjects", "No deleted subjects.")}
                      </TableCell>
                    </TableRow>
                  ) : (
